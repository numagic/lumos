from lumos.numpy.casadi_numpy.determine_type import *
import pytest
import numpy as np
...
def test_casadi():
    assert is_casadi_type(cas.GenMX_ones(5), recursive=False) == True
    assert is_casadi_type(cas.GenMX_ones(5), recursive=True) == True
...
def test_casadi_list():
    assert (
        is_casadi_type([cas.GenMX_ones(5), cas.GenMX_ones(5)], recursive=False) == False
    )
    assert (
        is_casadi_type([cas.GenMX_ones(5), cas.GenMX_ones(5)], recursive=True) == True
    )


def test_mixed_list():
    assert is_casadi_type([np.array(5), cas.GenMX_ones(5)], recursive=False) == False
    assert is_casadi_type([np.array(5), cas.GenMX_ones(5)], recursive=True) == True
